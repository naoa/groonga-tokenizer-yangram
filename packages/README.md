...
rpm -ivh http://packages.groonga.org/centos/groonga-release-1.1.0.2.noarch.rpm
yum update -y
...
cd /usr/local/src
rm -rf groonga-tokenizer-yangram-1.0.2
cp -rf groonga-tokenizer-yangram groonga-tokenizer-yangram-1.0.2
tar zcvf groonga-tokenizer-yangram-1.0.2.tar.gz groonga-tokenizer-yangram-1.0.2
mv groonga-tokenizer-yangram-1.0.2.tar.gz ~/rpmbuild/SOURCES/
rpmbuild -ba ~/rpmbuild/SPECS/groonga-tokenizer-yangram.spec
mkdir -p /usr/local/src/groonga-tokenizer-yangram/public/centos/6/
mv ~/rpmbuild/RPMS/x86_64/groonga-tokenizer-yangram-1.0.2-1.el6.x86_64.rpm /usr/local/src/groonga-tokenizer-yangram/public/centos/6/
```
...
mv ~/rpmbuild/RPMS/x86_64/groonga-tokenizer-yangram-1.0.2-1.el7.centos.x86_64.rpm /usr/local/src/groonga-tokenizer-yangram/public/centos/7/
```
...
rpm -ivh http://packages.groonga.org/fedora/groonga-release-1.1.0.2.noarch.rpm
yum update -y
yum install -y groonga-devel autoconf automake libtool wget tar gcc-c++ make mecab-devel libedit-devel rpm-build rpmdevtools
…
...
mv ~/rpmbuild/RPMS/x86_64/groonga-tokenizer-yangram-1.0.2-1.fc20.x86_64.rpm /usr/local/src/groonga-tokenizer-yangram/public/fedora/20/
```
...
cp -rf groonga-tokenizer-yangram groonga-tokenizer-yangram-1.0.2
tar zcvf groonga-tokenizer-yangram-1.0.2.tar.gz groonga-tokenizer-yangram-1.0.2
mv groonga-tokenizer-yangram-1.0.2.tar.gz ~/build/groonga-tokenizer-yangram_1.0.2.orig.tar.gz
cd ~/build
tar xfz groonga-tokenizer-yangram_1.0.2.orig.tar.gz
cd groonga-tokenizer-yangram-1.0.2/
cp -rf packages/debian .
debuild -us -uc -b
cd ..
mkdir -p /usr/local/src/groonga-tokenizer-yangram/public/debian/wheezy/
mv ~/build/groonga-tokenizer-yangram_1.0.2-1_amd64.deb /usr/local/src/groonga-tokenizer-yangram/public/debian/wheezy/
```
...
mv ~/build/groonga-tokenizer-yangram_1.0.2-1_amd64.deb /usr/local/src/groonga-tokenizer-yangram/public/debian/jessie/
```
...
mv ~/build/groonga-tokenizer-yangram_1.0.2-1_amd64.deb /usr/local/src/groonga-tokenizer-yangram/public/ubuntu/precise/
```
...
mv ~/build/groonga-tokenizer-yangram_1.0.2-1_amd64.deb /usr/local/src/groonga-tokenizer-yangram/public/ubuntu/trusty/
```
