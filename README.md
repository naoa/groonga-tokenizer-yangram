...
config_setコマンドで``tokenizer.yangram.#{lexicon_name}.skip_overlap_ratio`` を設定すると、出現頻度が低いレアトークンはスキップしなくなります。``GRN_II_CURSOR_SET_MIN_ENABLE``環境変数を設定するとより効果的です。
ただし、トークン数が増えることによるデメリットも大きいかもしれません。
