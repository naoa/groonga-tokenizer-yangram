...
スキップされません。すなわち、検索クエリ末尾では、オーバラップしている
トークンが含まれます。これはNgramのNに満たない1文字のトークンを含めてしまう
と検索性能が劣化するため、あえてスキップしないようにしています。

このほかアルファベットや数字などがグループされる字種境界においても1文字の
トークンが発生する可能性があります。しかし、字種境界の事前判定は難しい
ため、スキップの除外処理はしていません。したがって、字種境界においては
スキップされて1文字のトークンが発生することがあります。しかし、検索クエリ中
に字種境界が多数含まれる可能性は低くSplit系を使えば回避できるため今のところ
は無理にスキップ処理を飛ばすことは考えていません。
